document.addEventListener('DOMContentLoaded', () => {
    ...
    function getFramePath(index) {
        ...
        return `hero-banner/teaser${frameNum}.jpg`;
    }
    ...
    function updateHeroStickyTransition() {
        ...
        if (aboutRect.top < viewportHeight && aboutRect.bottom > 0) {
            stickyContainer.classList.add('hero-fixed');
            ...
            if (percentEntered >= 0.70) {
                // Outro triggers at 70% scrolled up
                if (textPhase5) textPhase5.classList.add('slide-up-outro');
                if (canvas) canvas.classList.add('no-blur');
                
                // Smoothly cross-fade opacity as it covers the remaining 30%
                const fadeProgress = (percentEntered - 0.7) / 0.3; // 0 to 1
                stickyContainer.style.opacity = String(1 - fadeProgress);
                stickyContainer.style.pointerEvents = 'none';
            } else {
                if (textPhase5) textPhase5.classList.remove('slide-up-outro');
                if (canvas) canvas.classList.remove('no-blur');
                stickyContainer.style.opacity = '1';
                stickyContainer.style.pointerEvents = 'none';
            }
        } else if (aboutRect.bottom <= 0) {
            // Completely scrolled past, hide completely
            stickyContainer.classList.remove('hero-fixed');
